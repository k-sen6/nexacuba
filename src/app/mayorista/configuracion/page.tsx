...
export default function MayoristaConfigPage() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      setPerfil(perf)
      const { data: may } = await supabase.from("mayoristas").select("*").eq("id", user.id).single()
      setMayorista(may)
      setLoading(false)
    })()
  }, [router])
  ...
  const fields = [
    { label: "Nombre del Negocio", value: mayorista?.nombre_negocio || "—" },
    { label: "WhatsApp", value: mayorista?.whatsapp || "—" },
    { label: "Provincia", value: mayorista?.provincia || "—" },
    ...
    <div className="min-h-screen bg-black pt-24 pb-16">
      <div className="max-w-7xl mx-auto px-5 sm:px-8">
        <div className="mb-10">
          ...
          <p className="text-gray-400 mt-1">Gestiona tu perfil, plan y facturación</p>
        </div>
        ...
        <div className="grid md:grid-cols-3 gap-6">
              ...
              <div className="flex items-center justify-between p-4 rounded-xl bg-gradient-to-r from-blue-600/10 to-purple-600/10 border border-blue-500/20">
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    <span className="text-2xl font-black text-white capitalize">{mayorista?.plan || "Gratis"}</span>
                  ...
                  <p className="text-sm text-gray-400">
                    {mayorista?.plan === "gratis" ? "Hasta 10 productos · Analytics básicos" : "Productos ilimitados · Analytics completos"}
                  </p>
                </div>
          ...
          <div className="space-y-4">
            {[
              { plan: "Básico", price: "$9.99", features: ["50 productos", "Analytics completos", "Soporte email"] },
              { plan: "Pro", price: "$19.99", features: ["200 productos", "Analytics avanzados", "Soporte prioritario", "API acceso"] },
              { plan: "Enterprise", price: "$49.99", features: ["Productos ilimitados", "Analytics en tiempo real", "Soporte 24/7", "API + Webhooks", "Equipo ilimitado"] },
            ].map((p) => (
